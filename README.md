...
Информация об общепринятых правилах создания сообщений к коммитам. Поскольку в вывод команды `git log --oneline` попадает только 72 
символа, то обычно стараются делать сообщение не больше 72 символов. Напомнили, что сообщения могут очень помочь. Например, чтобы
найти нужное изменение. Правила создания сообщений часто фрмируются свои в разных компаниях и командах. Часто сообщение начинают с 
идентификатора задачи в Джире, в рамках которой загружается коммит.

Есть проработанный стандарт *Conventional Commits*, который зачастую используется для ребозиториев для команд с открытым кодом. Он 
предлагает формат `<type>: <сообщение>`. **Type** это, например, *feat* или *fix*. В **GitHub** можно указать ссылку на задачу, 
заведённую в GitHub, тогда н свяжет коммит и задачу. Ссылка оформляется вот так: `#<номер задачи>`.

Для сообщений зачастую используют инфинитивы.
